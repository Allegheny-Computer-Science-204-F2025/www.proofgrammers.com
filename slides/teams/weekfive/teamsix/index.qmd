...
## One-Slide Contradiction Check

```{pyodide}
#| autorun: true
#| max-lines: 12
def weird_starts_with_z(checker):
    def F():
        return 'Z' if not checker(F) else 'N'
    return F

def test_case(checker_answer):
    def checker(_):
        return checker_answer
    F = weird_starts_with_z(checker)
    claimed = checker(F)
    actual = F().startswith('Z')
    return claimed, actual

print("🔍 Testing both possible answers:")
print("="*49)
for answer in [True, False]:
    claimed, actual = test_case(answer)
    status = "❌ CONTRADICTION" if claimed != actual else "✅ CONSISTENT"
    print(f"Claims: {claimed:5} | Actual: {actual:5} | {status}")
```

::: {.fragment .boxed-content}
**The contradiction is unavoidable** - every possible answer leads to a logical impossibility, proving that `startsWithZ` cannot exist.
:::
...
- **Static Analysis**  
  - Analyze the *code structure* without running it (just look at it).
  - Typicaly computable.
...
- Even problems that look simple can be **incomputable** when programs analyze their own behavior.
